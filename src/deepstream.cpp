...
namespace Machine_utilisation {
  void
  Manchine_util::update_fps(gint id) {
  ...
  int
  Manchine_util::create_input_sources(gpointer pipe, gpointer mux, guint num_sources) {
  ...
  void
  Manchine_util::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  Manchine_util::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data) {
  ...
  // adding the display machine utilisation while machine is utilising 

  void Manchine_util::addDisplayMachine_utilisation_data(gpointer batch_meta_data, gpointer frame_meta_data ,std::string start_time){
    // here i add the value to the mahine to the machine utilisation las 
    NvDsBatchMeta *batch_meta = (NvDsBatchMeta *)batch_meta_data;
    NvDsFrameMeta *frame_meta = (NvDsFrameMeta *)frame_meta_data;
    ...
    // if (txt_params->display_text)
    //   g_free (txt_params->display_text);
    txt_params->display_text = (char *)g_malloc0(200);

    update_fps(frame_meta->source_id);
    char* char_arrr;
    char_arrr = &start_time[0];

    offset = snprintf(txt_params->display_text, MAX_DISPLAY_LEN, "UTILISATION STARTS AT : %s" , char_arrr);

    /* Now set the offsets where the string should appear */
    txt_params->x_offset = 30;
    txt_params->y_offset = 50;
  ...
  GstPadProbeReturn
  Manchine_util::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                            gpointer u_data) {
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
      ...
      // here logic for the more susteainbility 
      if(start_time_locals != "None"){
          addDisplayMachine_utilisation_data(batch_meta , frame_meta , start_time_locals);
        }

        time_t end_time;
        time(&end_time);

        if(difftime(end_time , last_seen_instance) > 2 && start_time != 0){
          start_time = 0;
          start_time_locals = "None";
          cout<<"Restarted"<<endl;
        }
      ...
      for (l_obj = frame_meta->obj_meta_list; l_obj != NULL;
        ...
        if(class_index == 0) {
          if(start_time == 0){
            ...
            std::string start_time_local (std::asctime(now));
            start_time_locals = start_time_local;
          }
          changeBBoxColor(obj_meta, 1, 1.0, 0.0, 0.0, 0.25);
          
          addDisplayMachine_utilisation_data(batch_meta , frame_meta , start_time_locals);
          time(&last_seen_instance);
        }

        std::cout<<"This is the start time locals"<<start_time_locals<<std::endl;
  ...
  gboolean
  Manchine_util::bus_call(GstBus *bus, GstMessage *msg, gpointer data) {
  ...
  void
  Manchine_util::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data) {
  ...
  void
  Manchine_util::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data) {
  ...
  GstElement *
  Manchine_util::create_source_bin(guint index, gchar *uri) {
  ...
  gchar *
  Manchine_util::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path) {
  ...
  gboolean
  Manchine_util::set_tracker_properties(GstElement *nvtracker) {
    gboolean ret = FALSE;
    GError *error = NULL;
    gchar **keys = NULL;
    gchar **key = NULL;
  ...
  int
  Manchine_util::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
    ...
    g_object_set(G_OBJECT(pgie_yolo_detector),
                  "model-engine-file","model_b1_gpu0_fp32.engine", NULL);

    // Check if Engine Exists
    if (boost::filesystem::exists(boost::filesystem::path(
        Machine_utilisation::Manchine_util::PGIE_YOLO_ENGINE_PATH))) {
    ...
    if (!Machine_utilisation::Manchine_util::set_tracker_properties(nvtracker)) {
  ...
  void Manchine_util::setPaths(guint num_sources) {

    // Config Paths
    PGIE_YOLO_DETECTOR_CONFIG_FILE_PATH =
    strdup("YOLOV3_sheet_detector/config_infer_primary_yolov3.txt");

    TRACKER_CONFIG_FILE =
    strdup("tracker/ds_tracker_config.txt");
...
int main(int argc, char *argv[]) {
  ...
  Machine_utilisation::Manchine_util Manchine_util;
  GMainLoop *loop = NULL;
  ...
  ctx = g_option_context_new("Manchine_util DeepStream App");
  group = g_option_group_new("Manchine_util", NULL, NULL, NULL, NULL);
  g_option_group_add_entries(group, Manchine_util.entries);
  ...
  gint sources = Manchine_util.create_input_sources(pipeline, streammux, num_sources);
  if (sources == -1) {
    return -1;
  }
  else {
  ...
  Manchine_util.setPaths(num_sources);
  ...
  int fail_safe = Manchine_util.configure_element_properties(num_sources, streammux, pgie_yolo_detector,
  ...
  bus_watch_id = gst_bus_add_watch(bus, Manchine_util.bus_call, loop);
  ...
  if (!tiler_src_pad) {
    ...
    gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                      Manchine_util.tiler_src_pad_buffer_probe, NULL, NULL);
  }
